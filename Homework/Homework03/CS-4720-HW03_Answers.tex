...
\end{center}
\begin{center}
	\includegraphics[scale=1]{Figure_3_3_2}\\
	Figure 3.3.2
\end{center}
If we were to make the outside edges negative and the inside edge positive then we get the near the same results as before.  There ends up being two balanced triangles for with nodes ADC and nodes ABD.  However once again the triangle with nodes ABC form an unbalanced graph.  Therefore the overall graph is unbalanced.  Thus, we have shown that the node D can not be added to the graph with any kind of positive or negative connection that would make the overall graph a balanced complete graph.   \\
	\item PART B
	\item PART C
\end{enumerate}}
